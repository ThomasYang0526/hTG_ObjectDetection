...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 13 16:23:05 2021
...
class DeepSort(object):
    ...
    # def __init__(self, model_path):
        ...
        # self.extractor = Extractor('/home/thomas_yang/ML/CenterNet_TensorFlow2/deep/checkpoint/ckpt.t7', use_cuda=False)
    ...
    def update(self, bbox_xywh, confidences, ori_img):
        ...
        for track in self.tracker.tracks:
            ...
            if not track.is_confirmed() or track.time_since_update > 1:
                # print('continue', track.time_since_update, track.is_confirmed(), track)
                continue
            box = track.to_tlwh()
            x1,y1,x2,y2 = self._xywh_to_xyxy_centernet(box)
            track_id = track.track_id
            outputs.append(np.array([x1,y1,x2,y2,track_id], dtype=np.int))
            # print('test',x1,y1,x2,y2,track_id)
        if len(outputs) > 0:
    ...
    def _get_features(self, bbox_xywh, ori_img):
        ...
        for box in bbox_xywh:
            ...
            feature = np.reshape(cv2.resize(im, (64,128)), -1)
